# Most of the code made by Java#9999 -  Modified by hitsho
import pip 
try:
    import discord
...
def versionChecker():
    ...
    while True:
        response = requests.get(
            "https://pastebin.com/raw/YbiCvtC3"
        )
        if response:
...
print("Welcome to HitshoAutomation")
...
async def send_cookie_invalid_webhook(cookie_name, command_name):
    ...
    embed = discord.Embed(
        ...
        description=f" ``` The {cookie_name} has became invalid. Please update it by using the command !{command_name}. ```",
...
async def ss(ctx):
    ...
    image_bytes = BytesIO()
    ss.save(image_bytes, format='PNG')
    image_bytes.seek(0)
